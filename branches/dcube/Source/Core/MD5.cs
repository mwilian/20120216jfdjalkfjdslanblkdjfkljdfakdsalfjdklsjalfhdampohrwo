...
//MODIFIED SO IT GOES ON FLEXCEL NAMESPACE!
namespace FlexCel.Core
{
	internal sealed class MD5CryptoServiceProvider
	{
		private MD5CryptoServiceProvider(){}

		static internal MD5 Create(string hashName)
		{
			if(hashName == "MD5")
				return new MD5();
			else
	...
	internal class MD5 : IDisposable
	{
		...
		internal virtual void HashCore(byte[] input, int offset, int acount)
		{
            unchecked
            {
                int i;
                ...
                if((this.count[0] += (uint)((uint)(acount - offset) << 3)) < ((uint)(acount - offset) << 3))
                    this.count[1]++;
                this.count[1] += ((uint)(acount - offset) >> 29);

                partLen = 64 - index;

                // Transform as many times as possible.
                if ((acount - offset) >= partLen) 
                {
                    MD5_memcpy(this.buffer, index, input, offset, partLen);
                    Transform(this.buffer, 0);

                    for(i = partLen; i + 63 < (acount - offset); i += 64)
                        Transform(input, i);
                ...
                MD5_memcpy(this.buffer, index, input, i, (acount - offset)-i);
            }
		}
		...
		internal virtual byte[] HashFinal()
		{
        ...
        /// </summary>
        /// <returns></returns>
        internal virtual byte[] HashFinalEncryption()
        {
            byte[] digest =new byte[16];
        ...
        /// </summary>
        /// <returns></returns>
        internal virtual byte[] ComputeHashEncryption(byte[] abuffer)
        {
            HashCore(abuffer, 0, abuffer.Length);
            mHash = HashFinalEncryption();
            return mHash;
        }
		...
		private byte[] mHash;
		internal virtual bool CanReuseTransform 
		{
			get
			{
				return true;
		...
		internal virtual bool CanTransformMultipleBlocks 
		{
			get
			{
				return false;
		...
		internal byte[] ComputeHash(byte[] abuffer)
		{
			return ComputeHash(abuffer, 0, abuffer.Length);
		}
		internal byte[] ComputeHash(byte[] abuffer, int offset, int acount)
		{
			Initialize();
			HashCore(abuffer, offset, acount);
			mHash = HashFinal();
			return mHash;
		}
		...
		internal static int TransformBlock(
			byte[] inputBuffer,
			int inputOffset,
			int inputCount,
			byte[] outputBuffer,
		...
		internal static byte[] TransformFinalBlock(
			byte[] inputBuffer,
			int inputOffset,
			int inputCount
			)
		...
		{
			//nothing to dispose. This is IDIsposable to be compatible with standard MD5 implementation
		}
		public void Dispose()
		{
            Dispose(true);
