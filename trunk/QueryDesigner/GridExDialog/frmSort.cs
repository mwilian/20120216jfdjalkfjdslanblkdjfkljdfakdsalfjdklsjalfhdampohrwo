...
namespace QueryDesigner
{
	public partial class frmSort
	{
		...
		private void CreateSortKeys(GridEX grid)
		{
			...
			if (sortKeysCount > 0)
			{
                sortKeys[0] = CreateSortKey((GridEXColumn)((UIComboBoxItem)cboColumn0.SelectedItem).Value, optAscending0.Checked);
			}
			if (sortKeysCount > 1)
			{
				sortKeys[1] = CreateSortKey((GridEXColumn)((UIComboBoxItem)cboColumn1.SelectedItem).Value, optAscending1.Checked);
			}
			if (sortKeysCount > 2)
			{
                sortKeys[2] = CreateSortKey((GridEXColumn)((UIComboBoxItem)cboColumn2.SelectedItem).Value, optAscending2.Checked);
			}
			if (sortKeysCount > 3)
			{
                sortKeys[3] = CreateSortKey((GridEXColumn)((UIComboBoxItem)cboColumn3.SelectedItem).Value, optAscending3.Checked);
			}
		...
		private void FillColumnList(GridEXColumnCollection columns, Janus.Windows.EditControls.UIComboBox combo)
		{
            ...
            for (i = 0; i < columns.Count; i++)
			{
				...
				if (column.AllowSort)
				{
					combo.Items.Add(MainQD.GetColumnFriendlyName(column), column);
				}
			}
		}
		...
		private void SetSortKey(GridEXColumn column, bool ascending, UIComboBox combo, UIRadioButton optAscending, UIRadioButton optDescending)
		{
			...
			{
				foreach (Janus.Windows.EditControls.UIComboBoxItem item in combo.Items)
				{
